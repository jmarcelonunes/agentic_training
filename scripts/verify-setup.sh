...
check_typescript() {
    ...
    # Check TypeScript (avoid interactive npx prompt)
    if command -v tsc &> /dev/null; then
        TSC_VERSION=$(tsc --version | awk '{print $2}')
        check_pass "TypeScript $TSC_VERSION"
    elif [ -f "node_modules/typescript/package.json" ]; then
        TSC_VERSION=$(node -p "require('./node_modules/typescript/package.json').version" 2>/dev/null || echo "installed")
        check_pass "TypeScript $TSC_VERSION (local)"
    else
        check_warn "TypeScript not installed (global or local)"
    fi
